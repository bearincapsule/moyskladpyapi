...
def add_dropdown_and_formatting(file_path: str, sheet_name: str) -> None:
    ...
    max_row = ws.max_row  # Find the last used row
    max_column = ws.max_column  # Find the last used column (numeric index)

    # Convert column number to Excel column letter (e.g., 26 -> 'Z')
    last_column_letter = get_column_letter(max_column)


    # Add a dropdown menu for specified columns
    dv = DataValidation(type="list", formula1='"Да,Нет"', allow_blank=True)
    ws.add_data_validation(dv)
    for col in dropdown_columns:
        for row in range(2, max_row):  # rows 2 to 2000
            cell = f'{col}{row}'
            dv.add(ws[cell])
    ...
    # Apply auto-filter to the entire data range dynamically
    ...
    # Set general cell formatting and header style
...
async def fetch(
    ...
    global auth  # Use global authentication credentials
    login_attempts = 0  # Track login retries
        ...
        try:
            async with session.get(url, auth=auth) as response:
                if response.status == 401:
                    ...
                    # Ask the user for credentials again
                    new_username, new_password = get_credentials()
                    auth = BasicAuth(new_username, new_password)  # Update global auth
                    login_attempts += 1
...
async def fetch_product_details(
    ...
    # fields from the 'product' dict. Consider using a configuration
    # dict to map new fields if needed.
    """
...
# -------------------------------------------------------------------------------
# Fetch all products using pagination with a progress bar for batches
# -------------------------------------------------------------------------------
...
async def main():
    ...
    # Update overall progress bar total (9 major steps)
    overall_steps = 9

    # Prompt to ensure the Excel file is not open (for writing)
    ...
    async with ClientSession(timeout=timeout) as session:
        # Create overall progress bar for the main steps
        with tqdm(total=overall_steps, desc="Overall Progress", unit="step") as global_pbar:
            ...
            for chunk in chunkify(base_products, CHUNK_SIZE):
                ...
                # Current task progress bar for base products
                for coro in tqdm(
            ...
            for r in results:
                ...
                # Include each relevant price; extend here if needed
            ...
            # If file exists, manage sheet renaming
            if os.path.exists(filename):
                wb = openpyxl.load_workbook(filename)
                # Remove any existing "previous" sheet
                if "previous" in wb.sheetnames:
                    ws_prev = wb["previous"]
                    wb.remove(ws_prev)
                # Rename existing "current" sheet to "previous" if it exists
                if "current" in wb.sheetnames:
                    ws_current = wb["current"]
                    ws_current.title = "previous"
                wb.save(filename)
            # Now, write new data into a new "current" sheet
    ...
    # Optionally, try opening the Excel file (Windows only)
    try:
        os.startfile(filename)
    except Exception as e:
        logging.error(f"Could not open file {filename}: {e}")

    # Send email notification (remote log) upon successful run
